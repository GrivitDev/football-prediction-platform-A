...
function createAdId(): string {
  if (
    ...
    return `monetag-${crypto.randomUUID()}`;
  }

  return `monetag-${Date.now()}-${Math.random()
    .toString(36)
    .slice(2, 10)}`;
}

export class MonetagAdapter {
  private config:
    | MonetagConfig
    | null = null;
  ...
  async initialize(
    ...
    if (!config.publisherId) {
      throw new Error(
        'Monetag publisherId is required.',
      );
    }
  ...
  async load(
    ...
    if (!this.initialized) {
      throw new Error(
        'Monetag adapter has not been initialized.',
      );
    }
  ...
  private async mountAd(
    container: HTMLElement,
    request: ExternalAdRequest,
    options: MonetagAdRequestOptions,
  ): Promise<void> {
    if (!options.zoneId) {
      throw new Error(
        'Monetag zoneId is required.',
      );
    }
    ...
    /*
     * Monetag publisher-generated code
     ...
     * HTML or JavaScript supplied by metadata.
     */
  }
}